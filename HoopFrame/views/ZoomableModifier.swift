...
struct ZoomableModifier: ViewModifier {
    ...
    private var dragGesture: some Gesture {
        DragGesture()
            .onChanged { value in
                withAnimation(.interactiveSpring) {
                    transform = lastTransform.translatedBy(x: value.translation.width / max(transform.scaleX, .leastNonzeroMagnitude), y: value.translation.height / max(transform.scaleY, .leastNonzeroMagnitude))
                }
            }
            .onEnded { _ in
                onEndGesture()
    ...
    private func limitTransform(_ transform: CGAffineTransform) -> CGAffineTransform {
        ...
        if let maxZoomScale {
            ...
            if currentScale > maxZoomScale {
                let factor        : CGFloat           = maxZoomScale / currentScale
                let contentCenter : CGPoint           = CGPoint(x: contentSize.width / 2, y: contentSize.height / 2)
                let capTransform  : CGAffineTransform = CGAffineTransform.anchoredScale(scale: factor, anchor: contentCenter)
                capped = capped.concatenating(capTransform)
                self.model.scaleFactor = factor
            } else {
                self.model.scaleFactor = currentScale
            }
        }

        let maxX : CGFloat = contentSize.width * (capped.scaleX - 1)
        let maxY : CGFloat = contentSize.height * (capped.scaleY - 1)
        ...
        if capped.tx > 0
            ...
            || capped.ty < -maxY
        {
            let tx : CGFloat = min(max(capped.tx, -maxX), 0)
            let ty : CGFloat = min(max(capped.ty, -maxY), 0)
            capped.tx = tx
            capped.ty = ty
        }
        
        self.model.offsetX     = capped.tx
        self.model.offsetY     = capped.ty
...
extension View {
    @ViewBuilder
    public func zoomable(minZoomScale: CGFloat = 1, maxZoomScale: CGFloat? = nil, doubleTapZoomScale: CGFloat? = 3) -> some View {
        modifier(
            ZoomableModifier(minZoomScale: minZoomScale, maxZoomScale: maxZoomScale, doubleTapZoomScale: doubleTapZoomScale)
        )
    }

    @ViewBuilder
    public func zoomable(minZoomScale: CGFloat = 1, maxZoomScale: CGFloat? = nil, doubleTapZoomScale: CGFloat? = 3, outOfBoundsColor: Color = .clear) -> some View {
        GeometryReader { _ in
            ZStack {
                outOfBoundsColor
                self.zoomable(minZoomScale: minZoomScale, maxZoomScale: maxZoomScale, doubleTapZoomScale: doubleTapZoomScale)
            }
        }
    }
}
...
extension View {
    ...
    fileprivate func animatableTransformEffect(_ transform: CGAffineTransform) -> some View {
        scaleEffect(x: transform.scaleX, y: transform.scaleY, anchor: .zero).offset(x: transform.tx, y: transform.ty)
    }
}
